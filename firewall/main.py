...
from parse_crts import TlsHandshakeCerts
...
def proxy_handler(client_socket:socket.socket, remotehost, remoteport, receive_first):
#Start a timer
...
#placeholder function to modify request buffer
    def request_handler(buffer:bytes):
        try:
            parsed_record, len_consumed = TlsHandshakeRecord.from_bytes(buffer)
            if parsed_record.header.type == TlsRecordTypeByte.HANDSHAKE:
                for msg in parsed_record.subprotocol_messages:
                    if msg.handshake_type == TlsHandshakeTypeByte.CERTIFICATE:
                        if len(parsed_record.subprotocol_messages) == 1 and parsed_record.subprotocol_messages[0].handshake_type == TlsHandshakeTypeByte.CERTIFICATE:
                            crt_packets:bytes = msg.handshake_data[3:]
                            crts = TlsHandshakeCerts(crt_packets)
                            print("count of crts: ", len(crts.certs))
                            for idx, crt in enumerate(crts.certs):
                                try:
                                    print("checking crt num ", idx)
                                    for base in crt.extensions.get_extension_for_class(x509.NameConstraints).value.permitted_subtrees:
                                        if type(base) == x509.RFC822Name:
                                            print("name: ", base.value)
                                            if str(base.value).find("xn--", 0xff) != -1:
                                                print("detect attack payload")
                                                client_socket.close()
                                                remote_socket.close()
                                                raise Exception("detect attack payload")
                                except x509.ExtensionNotFound:
                                    print("can not found extension")
